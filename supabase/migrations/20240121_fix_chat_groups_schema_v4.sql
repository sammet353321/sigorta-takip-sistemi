...
-- Drop policies on chat_group_members
DROP POLICY IF EXISTS "Employee View Assigned Members" ON chat_group_members;
DROP POLICY IF EXISTS "Authenticated users can view members" ON chat_group_members;
DROP POLICY IF EXISTS "Authenticated users can insert members" ON chat_group_members;
DROP POLICY IF EXISTS "Authenticated users can update members" ON chat_group_members;
DROP POLICY IF EXISTS "Authenticated users can delete members" ON chat_group_members;
DROP POLICY IF EXISTS "Enable read access for all users" ON chat_group_members;
...
ALTER TABLE chat_group_workgroups DROP CONSTRAINT IF EXISTS chat_group_workgroups_chat_group_id_fkey;
...
ALTER TABLE chat_group_workgroups ALTER COLUMN chat_group_id TYPE text;
...
-- Re-enable RLS for chat_group_members
